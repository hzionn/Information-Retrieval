...
class VectorSpace:
    ...
    def buildQueryVector(self, termList:list[str]):
    ...
    def search(self, searchString: str, distance: str) -> list:
        """search for documents that match based on a list of terms"""
        cleaned = self.parser.tokenise(searchString)
        cleaned = self.parser.removeStopWords(cleaned)
        #print(f"query cleaned: {' '.join(cleaned)}")
        queryVector = self.buildQueryVector(cleaned)
        if distance == "cosine":
